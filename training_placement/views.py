...
def getPlacementData(request):
	...
	if request.session['role'] == "Admin":
		...
		placement_json = Placement.objects.filter(pl_status='0', pl_company=request.session['name']).values()
		data = list(placement_json)
		value = JsonResponse(data, safe=False)
		return value
...
def applyJob(request):
	...
	if 'web_email' in request.session:
		...
		if JobApplied.objects.filter(ja_email=request.session['web_email'], ja_jb_id=request.POST['txtJobID'], ja_status='0').exists(): 
...
def applyWorkshop(request):
	...
	if 'web_email' in request.session:
		...
		if WorkshopApplied.objects.filter(wa_email=request.session['web_email'], wa_wr_id=request.POST['txtWorkshopID'], wa_status='0').exists(): 
			return HttpResponse(100)
		else:
